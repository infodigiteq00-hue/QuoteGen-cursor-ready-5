...
<body>
  ...
  ${(() => {
    const termsBody = String(quote?.fields?.standardTerms || profile?.standardTerms || '')
      .replace(/<[^>]+>/g, ' ')
      .replace(/\s+/g, ' ')
      .trim()
    const notesBody = notes.filter(Boolean)
    const termsTitle = escapeHtml(quote?.fields?.standardTermsTitle || 'STANDARD TERMS')
    const notesTitle = escapeHtml(quote?.fields?.notesTitle || 'NOTES')
    const widthPct = Math.max(40, Math.min(100, Number(quote?.fields?.termsBlockWidthPct) || 50))
    const stackNotes = widthPct >= 80
    if (!termsBody && !notesBody.length) return ''
    if (stackNotes || !termsBody || !notesBody.length) {
      let html = ''
      if (termsBody) {
        html += `<div style="margin-top:28px;width:${widthPct}%;max-width:100%;">
    <div style="color:${accent};font-size:9pt;font-weight:800;letter-spacing:.1em;">${termsTitle}</div>
    <div style="color:${muted};white-space:pre-line;margin-top:6px;">${escapeHtml(termsBody)}</div>
  </div>`
      }
      if (notesBody.length) {
        html += `<div style="margin-top:${termsBody ? 14 : 28}px;width:${widthPct}%;max-width:100%;">
    <div style="color:${accent};font-size:9pt;font-weight:800;letter-spacing:.1em;">${notesTitle}</div>
    <div style="margin-top:6px;">${notesBody.map(n => `<div>${escapeHtml(n)}</div>`).join('')}</div>
  </div>`
      }
      return html
    }
    return `<table style="width:100%;border-collapse:collapse;margin-top:28px;">
    <tr>
      <td style="width:${widthPct}%;vertical-align:top;padding-right:16px;">
        <div style="color:${accent};font-size:9pt;font-weight:800;letter-spacing:.1em;">${termsTitle}</div>
        <div style="color:${muted};white-space:pre-line;margin-top:6px;">${escapeHtml(termsBody)}</div>
      </td>
      <td style="vertical-align:top;">
...
async function fillQuotationDataSheet(wb, { quote, profile, columns, totals, theme, docLabel, sheetName }) {
  ...
  r += 2
  const termsText = String(quote?.fields?.standardTerms || profile?.standardTerms || '').trim()
  const notesText = (quote?.notes || []).filter(Boolean).join('\n').trim()
  if (termsText) {
    sheet.getCell(r, 1).value = String(quote?.fields?.standardTermsTitle || 'STANDARD TERMS').toUpperCase()
    sheet.getCell(r, 1).font = { bold: true, color: { argb: `FF${accent}` }, size: 10 }
    r += 1
    sheet.mergeCells(r, 1, r, colCount)
    sheet.getCell(r, 1).value = termsText
    sheet.getCell(r, 1).alignment = { wrapText: true }
    r += 2
  }
  if (notesText) {
    sheet.getCell(r, 1).value = String(quote?.fields?.notesTitle || 'NOTES').toUpperCase()
    sheet.getCell(r, 1).font = { bold: true, color: { argb: `FF${accent}` }, size: 10 }
    r += 1
    sheet.mergeCells(r, 1, r, colCount)
    sheet.getCell(r, 1).value = notesText
    sheet.getCell(r, 1).alignment = { wrapText: true }
    r += 2
  }
