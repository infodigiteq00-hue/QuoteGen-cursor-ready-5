...
function commercialRoleValue(quote, role) {
  const termsKey = roleToTermsKey(role)
  if (termsKey) return quote?.terms?.[termsKey] || ''
  if (role === 'clarifications') return (quote?.clarifications || []).filter(Boolean).join('\n')
  return ''
}

const COMMERCIAL_ROLES = ['payment_terms', 'delivery_terms', 'validity_terms', 'freight_terms', 'tax_terms', 'clarifications']

function syncCommercialRole(role, v, update) {
  const termsKey = roleToTermsKey(role)
  if (termsKey) update(['terms', termsKey], v)
  else if (role === 'clarifications') update(['clarifications'], String(v).split('\n').filter(Boolean))
}

function inferExcelCellRole(cell, row) {
  if (cell?.role && cell.role !== 'content') return cell.role
  const prefixRole = cell?.labelPrefix
    ? excelLabelRole(String(cell.labelPrefix).replace(/[:\-–]\s*$/, ''))
    : null
  if (prefixRole) return prefixRole
  const left = (row?.cells || [])
    .filter(c => Number(c.col) < Number(cell.col))
    .sort((a, b) => Number(b.col) - Number(a.col))[0]
  if (left?.value) {
    const role = excelLabelRole(left.value)
    if (role) return role
  }
  return cell?.role || null
}

function buildWordSuggestMeta(node, root, { suggestClients, suggestProducts, pickClient, applyProduct }) {
  ...
  const custField = role ? customerFieldFromRole(role) : null
  if (custField) {
    return {
      kind: 'client',
      items: (query) => clientSuggestionItems(suggestClients, query, custField),
      onPick: (item) => pickClient(item.client)
    }
  }
  if (field && Number.isInteger(itemIndex) && field !== '__sr__' && field !== 'sr') {
    const colId = field === 'specification' ? 'specification' : (field === 'item' ? 'description' : field)
    if (['description', 'specification', 'item'].includes(field) || field === colId) {
      return {
        kind: 'product',
        itemIndex,
        colId,
        items: (query) => productSuggestionItems(suggestProducts, query),
        onPick: (item) => applyProduct(itemIndex, item.product, colId)
      }
    }
  }
  const cell = node.closest?.('td,th')
  if (!cell || !root.contains(cell)) return null
  const tdField = cell.getAttribute('data-qg-field')
  const tdItemIndex = Number(cell.getAttribute('data-qg-item'))
  if (tdField && Number.isInteger(tdItemIndex) && tdField !== '__sr__' && tdField !== 'sr') {
    const colId = tdField === 'specification' ? 'specification' : (tdField === 'item' ? 'description' : tdField)
    return {
      kind: 'product',
      itemIndex: tdItemIndex,
      colId,
      items: (query) => productSuggestionItems(suggestProducts, query),
      onPick: (item) => applyProduct(tdItemIndex, item.product, colId)
    }
  }
  ...
  const labelText = cells[idx - 1]?.innerText?.replace(/\u00a0/g, ' ').trim() || ''
  const inferredRole = layoutFieldRole(labelText)
  const inferredCust = inferredRole ? customerFieldFromRole(inferredRole) : null
  if (inferredCust) {
    return {
      kind: 'client',
      items: (query) => clientSuggestionItems(suggestClients, query, inferredCust),
      onPick: (item) => pickClient(item.client)
    }
  }
  return null
...
function ExcelInlineCell({
  ...
  const effectiveRole = inferExcelCellRole(cell, row)
  const custField = customerFieldFromRole(effectiveRole)
  ...
  const syncRole = (role, v) => {
    ...
    else if (role === 'enquiry_ref') update(['fields', 'referenceNo'], v)
    else if (role === 'subject') update(['title'], v)
    else if (role === 'notes') update(['notes'], String(v).split('\n'))
    else if (COMMERCIAL_ROLES.includes(role)) syncCommercialRole(role, v, update)
  ...
  if (!hasLayoutEdit) {
    ...
    } else if (effectiveRole === 'valid_until') {
      value = undoubleText(quote.fields?.validUntil || quote.validUntil || rawValue)
      onChange = (v) => syncRole('valid_until', v)
    } else if (effectiveRole === 'enquiry_ref') {
      value = undoubleText(quote.fields?.referenceNo || quote.referenceNo || rawValue)
      onChange = (v) => syncRole('enquiry_ref', v)
    } else if (effectiveRole === 'subject') {
      value = undoubleText(quote.title ?? rawValue)
      onChange = (v) => syncRole('subject', v)
    } else if (effectiveRole === 'notes') {
      value = undoubleText((quote.notes || []).filter(Boolean).join('\n') || rawValue)
      onChange = (v) => syncRole('notes', v)
      multiline = true
    } else if (COMMERCIAL_ROLES.includes(effectiveRole)) {
      value = undoubleText(commercialRoleValue(quote, effectiveRole) || rawValue)
      onChange = (v) => syncRole(effectiveRole, v)
      multiline = effectiveRole === 'clarifications' || /\n/.test(String(value || ''))
    }
  } else if (custField || ['quote_number', 'date', 'valid_until', 'enquiry_ref', 'subject', 'notes', ...COMMERCIAL_ROLES].includes(effectiveRole)) {
    onChange = (v) => syncRole(effectiveRole === 'notes' ? 'notes' : effectiveRole, v)
    if (effectiveRole === 'notes' || effectiveRole === 'clarifications') multiline = true
...
/** Floating suggest menu for Word layout contentEditable fields (clients + line items). */
function WordLayoutSuggest({
  rootRef,
  suggestClients,
  suggestProducts,
  pickClient,
  ...
  useEffect(() => {
    ...
    const resolveMeta = (node) => buildWordSuggestMeta(node, root, {
      suggestClients,
      suggestProducts,
      pickClient,
      applyProduct
    ...
    const onFocusIn = (e) => {
      const node = e.target.closest?.('[data-slot], [data-qg-field], td, th')
      if (!node || !root.contains(node)) { close(); return }
      if (node.getAttribute?.('data-slot') === 'total') { close(); return }
      const meta = resolveMeta(node)
      if (!meta) { close(); return }
      activeRef.current = node
      metaRef.current = meta
      const text = (node.innerText || node.textContent || '').replace(/\u00a0/g, ' ').trim()
      positionMenu(node, meta.items(text))
    }

    const onInput = (e) => {
      const node = activeRef.current
      const meta = metaRef.current
      if (!node || !meta || !root.contains(e.target)) return
      const text = (node.innerText || node.textContent || '').replace(/\u00a0/g, ' ').trim()
      positionMenu(node, meta.items(text))
    }

    const onMouseDown = (e) => {
      if (e.target.closest?.('.qg-suggest-list')) return
  ...
  }, [rootRef, suggestClients, suggestProducts, pickClient, applyProduct])

  if (!menu?.items?.length) return null

  const handlePick = (item) => {
    const meta = metaRef.current
    if (meta?.onPick) meta.onPick(item)
    setMenu(null)
    activeRef.current = null
    metaRef.current = null
  }
  ...
  return createPortal(
    <div
      className="qg-suggest qg-suggest--open no-print"
      style={{
        position: 'absolute',
        top: menu.top,
        left: menu.left,
...
export default function UploadedTemplateQuote({
  ...
  const [baseSheets, setBaseSheets] = useState(() => {
    const raw = template.content?.sheets || []
    return raw.length ? scrubTransientExcelShell(structuredClone(raw)) : []
  })
  ...
  const pickClient = (client) => {
    if (!client) return
    const shipAddrs = client.shippingAddresses || []
    const nextCustomer = {
      ...(quote.customer || {}),
      ...
      shippingAddresses: shipAddrs.length ? shipAddrs : quote.customer?.shippingAddresses
    }
    if (shipAddrs.length && quote.customer?.shippingSame === false) {
      nextCustomer.shippingLocation = shipAddrs[0]
    }
  ...
  useEffect(() => {
    const raw = template.content?.sheets || []
    setBaseSheets(raw.length ? scrubTransientExcelShell(structuredClone(raw)) : [])
  ...
  const syncWordPageFromDom = (pageIndex) => {
    ...
    el.querySelectorAll('[data-slot]').forEach((node) => {
      ...
      } else if (role === 'enquiry_ref') {
        const cur = quote.fields?.referenceNo || quote.referenceNo || ''
        if (text !== String(cur)) update(['fields', 'referenceNo'], text)
      ...
      else if (role === 'clarifications' && text !== (quote.clarifications || []).filter(Boolean).join('\n')) {
        update(['clarifications'], text.split('\n').filter(Boolean))
      } else if (roleToTermsKey(role)) {
        const key = roleToTermsKey(role)
        const cur = quote.terms?.[key] || ''
        if (text !== String(cur)) update(['terms', key], text)
      }
    })
  }
    ...
    <main className="min-h-screen text-ink print:bg-white" style={{ background: design.pageBg || '#edf1ed' }}>
      ...
      {type === 'word' ? (
        <section className="upload-layout-scroll mx-auto overflow-x-auto p-3 pb-12 sm:p-7" style={{ width: '100%', maxWidth: '100%' }}>
          ...
          <div className="upload-word-stack" ref={wordStackRef} style={{ width: pageWidthPx, minWidth: pageWidthPx }}>
        ...
        <section className="upload-layout-scroll overflow-x-auto p-3 pb-12 sm:p-5" style={{ width: '100%', maxWidth: '100%' }}>
          {sheet ? (
            ...
            <div
              className="upload-excel-paper relative rounded-lg shadow-soft bg-white"
              style={{
                width: pageWidthPx,
            ...
            >
              {sheet.images?.map((img) => (
                <img
                  ...
                  style={{
                    left: 56 + (img.leftPx ?? ((img.fromCol * 64) + (img.colOff ? img.colOff / 9525 : 0))),
