...
function formatMappingHint(mapping) {
  ...
  const skip = new Set(['formulas', 'header_footer', 'images', 'total', 'line_items', 'line_cell'])
  const fields = summary.filter(s => s.kind === 'field' && !skip.has(s.role)).map(s => s.label)
  const cells = summary.filter(s => s.kind === 'cell' && !skip.has(s.role)).map(s => s.label)
  const cols = summary.filter(s => s.kind === 'column')
  const uniqueFields = [...new Set([...fields, ...cells])]
  const parts = []
  if (uniqueFields.length) parts.push(uniqueFields.join(', '))
  if (cols.length) parts.push(`${cols.length} line-item column${cols.length === 1 ? '' : 's'}`)
  return parts.length ? `Mapped for auto-fill: ${parts.join(' · ')}` : ''
}
