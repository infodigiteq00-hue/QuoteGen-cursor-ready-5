  ...
  const referenceField = (
    <InlineField
      ...
      style={{
        minWidth: 128,
        padding: '3px 8px',
        border: `1px solid ${theme.tableBorder || '#e2e8f0'}`,
        borderRadius: 6,
        background: '#fff'
      }}
    />
  ...
  @page qg-studio { size: ${pageSize}; margin: 0; page-orientation: upright; }
      ...
      <div className="qg-studio-paper-frame" style={{ width }}>
        {pages.map((page, i) => (
          <article
            key={i}
            className={`qg-studio-paper quote-paper ${theme.themeClass || ''} ${i === 0 ? 'qg-studio-paper--first' : 'qg-studio-paper--continued'}`}
            data-qg-theme={themeId}
            style={paperVars}
          ...
          >
            ...
            <div className="qg-paper-plate">
              {watermarkEnabled && watermarkUrl ? (
                <div className="qg-paper-watermark" aria-hidden="true">
                  <img src={watermarkUrl} alt="" onError={onQuoteAssetImgError} />
                </div>
              ) : null}
              <div className="qg-paper-content">
                {page}
              </div>
            </div>
            <SheetRunFooter
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <label className="flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600">
          <input
            ...
            disabled={!logoUrl}
            title={logoUrl ? 'Show your logo faintly behind the quotation' : 'Upload a company logo to enable watermark'}
          />
          Watermark
        </label>
