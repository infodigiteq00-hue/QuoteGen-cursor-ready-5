...
import { inferTemplatePageWidth, summarizeTemplateMapping } from '../shared/templateMap.js'
import {
...
import { readUploadFileBuffer, readUploadFileMeta, saveUploadFile } from './uploadFileStorage.js'
import { onlyOfficeEnabled } from './onlyoffice.js'
import { registerUploadTemplateRoutes, scrubWordHtml, scrubExcelSheets } from './uploadTemplates.js'
...
const STANDARD_INDEXED_PRIMARIES = new Set([
  'FF0000', '00FF00', '0000FF', 'FFFF00', 'FF00FF', '00FFFF'
])

async function parseWorkbookIndexedColors(buffer) {
  const palette = { ...INDEXED_COLORS }
  try {
    const zip = await JSZip.loadAsync(buffer)
    const xml = await zip.file('xl/styles.xml')?.async('string')
    if (!xml) return palette
    const block = String(xml).match(/<(?:\w+:)?indexedColors\b[^>]*>([\s\S]*?)<\/(?:\w+:)?indexedColors>/i)?.[1]
    if (!block) return palette
    const rgbs = [...block.matchAll(/rgb="([0-9A-Fa-f]{6,8})"/gi)].map(m => m[1].slice(-6).toUpperCase())
    rgbs.forEach((hex, offset) => {
      palette[8 + offset] = hex
    })
  } catch { /* optional */ }
  return palette
}

function resolveIndexedColor(index, palette = INDEXED_COLORS) {
  const i = Number(index)
  if (!Number.isFinite(i)) return null
  let hex = palette[i] ?? INDEXED_COLORS[i]
  if (!hex) return null
  hex = String(hex).replace(/^#/, '').slice(-6).toUpperCase()

  // Some exporters (incl. AI-generated xlsx) put extended palette colors at index+8
  // while still using legacy slot numbers 8–15 (shows as garish red/magenta in browsers).
  if (i >= 8 && i <= 15 && STANDARD_INDEXED_PRIMARIES.has(hex)) {
    const extended = palette[i + 8]
    if (extended && String(extended).toUpperCase() !== hex) {
      hex = String(extended).replace(/^#/, '').slice(-6).toUpperCase()
    }
  }
...
function parseWorkbookTheme(workbook) {
  ...
  if (!scheme) return OFFICE_THEME.slice()
  const slots = ['dk1', 'lt1', 'dk2', 'lt2', 'accent1', 'accent2', 'accent3', 'accent4', 'accent5', 'accent6', 'hlink', 'folHlink']
  return slots.map((slot, i) => {
    const block = scheme.match(new RegExp(`<a:${slot}\\b[^>]*>([\\s\\S]*?)<\\/a:${slot}>`, 'i'))?.[1]
    if (!block) return OFFICE_THEME[i]
    const sys = block.match(/lastClr="([0-9A-Fa-f]{6})"/i)?.[1]
    if (sys) return sys.toUpperCase()
    const rgb = block.match(/val="([0-9A-Fa-f]{6})"/i)?.[1]
    return rgb ? rgb.toUpperCase() : OFFICE_THEME[i]
  })
}

function excelColor(color, theme = OFFICE_THEME, indexedPalette = INDEXED_COLORS) {
  if (!color) return null
  if (color.argb) return argbToCss(color.argb)
  if (color.rgb) return argbToCss(color.rgb)
  if (color.theme != null) {
  ...
  if (color.indexed != null) {
    return resolveIndexedColor(color.indexed, indexedPalette)
  }
  return null
}

function borderSide(side, theme, indexedPalette) {
  if (!side || side.style === 'none' || !side.style) return null
  const color = excelColor(side.color, theme, indexedPalette) || '#000000'
  const width =
...
function extractFillColor(fill, theme, indexedPalette) {
  if (!fill || fill.type === 'none') return null
  if (fill.type === 'pattern' && fill.pattern === 'none') return null
  const fg = excelColor(fill.fgColor, theme, indexedPalette)
  const bg = excelColor(fill.bgColor, theme, indexedPalette)
  if (fill.type === 'pattern') return fg || bg
  return fg || bg
}

function extractCellStyle(cell, theme, indexedPalette) {
  ...
  const bg = extractFillColor(fill, theme, indexedPalette)

  const borderTop = borderSide(border.top, theme, indexedPalette)
  const borderRight = borderSide(border.right, theme, indexedPalette)
  const borderBottom = borderSide(border.bottom, theme, indexedPalette)
  const borderLeft = borderSide(border.left, theme, indexedPalette)
  ...
  return {
    ...
    color: excelColor(font.color, theme, indexedPalette),
    backgroundColor: bg,
...
const WORD_THEME_KEYS = ['dk1', 'lt1', 'dk2', 'lt2', 'accent1', 'accent2', 'accent3', 'accent4', 'accent5', 'accent6', 'hlink', 'folHlink']
const WORD_THEME_ALIAS = { tx1: 'dk1', bg1: 'lt1', tx2: 'dk2', bg2: 'lt2', dark1: 'dk1', light1: 'lt1', dark2: 'dk2', light2: 'lt2' }

function parseWordThemeXml(xml) {
  const theme = {}
  const scheme = String(xml || '').match(/<a:clrScheme\b[^>]*>([\s\S]*?)<\/a:clrScheme>/i)?.[1] || ''
  for (const key of WORD_THEME_KEYS) {
    const block = scheme.match(new RegExp(`<a:${key}\\b[^>]*>([\\s\\S]*?)</a:${key}>`, 'i'))?.[1] || ''
    const hex = block.match(/(?:lastClr|val)="([0-9A-Fa-f]{6})"/)?.[1]
    if (hex) theme[key] = `#${hex.toUpperCase()}`
  }
  return theme
}

function applyWordTintShade(hex, tintHex, shadeHex) {
  const raw = String(hex || '').replace('#', '')
  if (raw.length < 6) return hex || null
  let r = parseInt(raw.slice(0, 2), 16)
  let g = parseInt(raw.slice(2, 4), 16)
  let b = parseInt(raw.slice(4, 6), 16)
  const mix = (tr, tg, tb, amount) => {
    r = Math.round(r + (tr - r) * amount)
    g = Math.round(g + (tg - g) * amount)
    b = Math.round(b + (tb - b) * amount)
  }
  if (tintHex) mix(255, 255, 255, Math.min(1, parseInt(String(tintHex), 16) / 255))
  if (shadeHex) mix(0, 0, 0, Math.min(1, parseInt(String(shadeHex), 16) / 255))
  const to = (n) => Math.max(0, Math.min(255, n)).toString(16).padStart(2, '0')
  return `#${to(r)}${to(g)}${to(b)}`
}

function resolveWordThemeColor(el, theme = {}) {
  if (!el) return null
  const fill = xmlAttr(el, 'fill') || xmlAttr(el, 'val')
  if (fill && fill.toLowerCase() !== 'auto') return `#${fill.replace(/^#/, '')}`
  const name = WORD_THEME_ALIAS[xmlAttr(el, 'themeFill')]
    || xmlAttr(el, 'themeFill')
    || WORD_THEME_ALIAS[xmlAttr(el, 'themeColor')]
    || xmlAttr(el, 'themeColor')
  const key = String(name || '').toLowerCase()
  const base = theme[key]
  if (!base) return null
  return applyWordTintShade(
    base,
    xmlAttr(el, 'themeFillTint') || xmlAttr(el, 'themeTint'),
    xmlAttr(el, 'themeFillShade') || xmlAttr(el, 'themeShade')
  )
}

function parseTblBorders(tblXml) {
  const block = String(tblXml || '').match(/<w:tblBorders\b[\s\S]*?<\/w:tblBorders>/i)?.[0] || ''
  const out = {}
  for (const side of ['top', 'left', 'bottom', 'right', 'insideH', 'insideV']) {
    const el = block.match(new RegExp(`<w:${side}\\b[^>]*\\/?>`))?.[0]
    const css = parseWordBorder(el)
    if (css && css !== 'none') out[side] = css
  }
  return out
}

function parseWordTableSpec(tblXml, theme = {}) {
  const tblW = tblXml.match(/<w:tblW\b[^>]*\/?>/)?.[0]
  const tblBorders = parseTblBorders(tblXml)
  const cells = []
  let hasBorders = Boolean(tblBorders.top || tblBorders.left || tblBorders.bottom || tblBorders.right || tblBorders.insideH || tblBorders.insideV)
  ...
  const parsedRows = trBlocks.map((tr) => {
    const row = []
    for (const tc of collectBlocks(tr, 'w:tc')) {
      ...
      const cell = {
        ...
        background: resolveWordThemeColor(shd, theme)
      }
  ...
  parsedRows.forEach((row, ri) => {
    row.forEach((cell, ci) => {
      if (!cell.top) cell.top = (ri === 0 ? tblBorders.top : tblBorders.insideH) || null
      if (!cell.bottom) cell.bottom = (ri === parsedRows.length - 1 ? tblBorders.bottom : tblBorders.insideH) || null
      if (!cell.left) cell.left = (ci === 0 ? tblBorders.left : tblBorders.insideV) || null
      if (!cell.right) cell.right = (ci === row.length - 1 ? tblBorders.right : tblBorders.insideV) || null
      if (cell.top || cell.left || cell.bottom || cell.right) hasBorders = true
      cells.push(cell)
    })
  })
...
function extractWordTableSpecs(xml, theme = {}) {
  return collectBlocks(xml, 'w:tbl').map((tbl) => parseWordTableSpec(tbl, theme))
}
...
function extractWordParagraphLooks(xml, theme = {}) {
  const looks = []
  for (const p of collectBlocks(xml, 'w:p')) {
    const text = wordXmlText(p)
    if (!text) continue
    const pPr = p.match(/<w:pPr\b[\s\S]*?<\/w:pPr>/i)?.[0] || ''
    const firstRun = p.match(/<w:r\b[\s\S]*?<\/w:r>/i)?.[0] || ''
    const rPr = firstRun.match(/<w:rPr\b[\s\S]*?<\/w:rPr>/i)?.[0] || pPr.match(/<w:rPr\b[\s\S]*?<\/w:rPr>/i)?.[0] || ''
    const colorEl = rPr.match(/<w:color\b[^>]*\/?>/)?.[0]
    const szEl = rPr.match(/<w:sz\b[^>]*\/?>/)?.[0]
    const jc = pPr.match(/<w:jc\b[^>]*\/?>/)?.[0]
    const sz = Number(xmlAttr(szEl, 'val'))
    looks.push({
      text,
      color: resolveWordThemeColor(colorEl, theme),
      fontSizePt: Number.isFinite(sz) && sz > 0 ? sz / 2 : null,
      align: xmlAttr(jc, 'val') || null,
      bold: /<w:b\b(?![^>]*w:val="(?:0|false)")/i.test(rPr)
    })
  }
  return looks
}

function applyWordParagraphLooks(html, looks) {
  if (!looks?.length) return html
  let i = 0
  return String(html || '').replace(/<(p|h[1-6])(\b[^>]*)>([\s\S]*?)<\/\1>/gi, (full, tag, attrs, inner) => {
    const text = String(inner).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()
    if (!text) return full
    let look = null
    for (let k = i; k < looks.length; k++) {
      const candidate = looks[k].text
    ...
    if (!look) return full
    const bits = []
    if (look.color) bits.push(`color:${look.color}`)
    if (look.fontSizePt) bits.push(`font-size:${look.fontSizePt}pt`)
    if (look.align && look.align !== 'left') bits.push(`text-align:${look.align === 'both' ? 'justify' : look.align}`)
    if (look.bold) bits.push('font-weight:700')
    if (!bits.length) return full
    return `<${tag}${mergeStyleAttr(attrs, bits.join(';'))}>${inner}</${tag}>`
  })
}
...
export async function parseExcel(buffer, fileName) {
  const workbook = new ExcelJS.Workbook()
  await workbook.xlsx.load(buffer)
  const theme = parseWorkbookTheme(workbook)
  const indexedPalette = await parseWorkbookIndexedColors(buffer)
  ...
  workbook.eachSheet((worksheet) => {
    ...
    for (let r = 1; r <= rowCount; r++) {
      ...
      for (let c = 1; c <= colCount; c++) {
        ...
        const style = extractCellStyle(cell, theme, indexedPalette)
...
export async function parseWord(buffer, fileName) {
  const zip = await JSZip.loadAsync(buffer)
  const docFile = zip.file('word/document.xml')
  let docXml = docFile ? await docFile.async('string') : ''
  if (docXml) {
    docXml = injectWordPageBreakMarkers(docXml)
  ...
  let theme = {}
  try {
    const themeName = Object.keys(zip.files).find((n) => /word\/theme\/theme\d+\.xml$/i.test(n))
    const themeFile = zip.file('word/theme/theme1.xml') || (themeName ? zip.file(themeName) : null)
    if (themeFile) theme = parseWordThemeXml(await themeFile.async('string'))
  } catch { /* theme is optional */ }
  const tableSpecs = docXml ? extractWordTableSpecs(docXml, theme) : []
  html = applyWordTableSpecs(html, tableSpecs)
  html = applyWordParagraphLooks(html, extractWordParagraphLooks(docXml, theme))
  ...
  html = normalizeWordPageBreaks(html)
  const pages = splitWordHtmlPages(html)
  const design = {
  ...
  return {
    type: 'word',
    fileName,
    html: pages.length > 1 ? joinWordHtmlPages(pages) : html,
...
function imgSrcFromTag(tag) {
  return String(tag || '').match(/\bsrc\s*=\s*(["'])([\s\S]*?)\1/i)?.[2] || ''
}

/** True when saved HTML is missing logos or has broken image data-URLs. */
function wordHtmlNeedsImageRefresh(html) {
  const tags = [...String(html || '').matchAll(/<img\b[^>]*>/gi)].map(m => m[0])
...
/** Swap broken/missing img src from a fresh docx parse while keeping saved layout edits. */
function mergeWordImages(savedHtml, freshHtml) {
  const freshImgs = [...String(freshHtml || '').matchAll(/<img\b[^>]*>/gi)].map(m => m[0])
  if (!freshImgs.length) return savedHtml || freshHtml
  const saved = String(savedHtml || '')
  if (!/<img\b/i.test(saved)) return freshHtml
  let i = 0
  return saved.replace(/<img\b[^>]*>/gi, (tag) => {
    const src = imgSrcFromTag(tag)
    if (src.startsWith('data:image') && src.length >= 200) return tag
    const next = freshImgs[i++] || freshImgs[freshImgs.length - 1]
    return next || tag
  })
}

/**
 * Re-read the original .docx/.xlsx when saved preview HTML lost embedded images
 * (common after Supabase sync or editing in OnlyOffice without re-exporting HTML).
 */
export async function refreshTemplateLayoutFromFile(tpl) {
  const fileId = tpl?.content?.fileId
  if (!fileId) return tpl

  const buf = await readUploadFileBuffer(fileId)
  if (!buf?.length) return tpl

  const meta = await readUploadFileMeta(fileId)
  const fileName = meta?.fileName || tpl.sourceFileName || (tpl.type === 'excel' ? 'book.xlsx' : 'doc.docx')

  try {
    if (tpl.type === 'word') {
      const savedHtml = tpl.content?.html || ''
      const parsed = await parseWord(buf, fileName)
      const freshHtml = parsed.html || ''
      if (!freshHtml) return tpl
      if (!savedHtml.trim() || !/<img\b/i.test(savedHtml)) {
        tpl.content = {
          ...tpl.content,
          html: freshHtml,
          ...(parsed.pages?.length > 1 ? { pages: parsed.pages } : {})
        }
      } else if (wordHtmlNeedsImageRefresh(savedHtml)) {
        tpl.content = {
          ...tpl.content,
          html: mergeWordImages(savedHtml, freshHtml)
        }
      } else {
        return tpl
      }
      tpl.design = { ...(tpl.design || {}), ...(parsed.design || {}) }
    } else if (tpl.type === 'excel') {
      const parsed = await parseExcel(buf, fileName)
      const freshSheets = parsed.sheets || []
      if (!freshSheets.length) return tpl
      const savedSheets = tpl.content?.sheets || []
      if (!savedSheets.length) {
        tpl.content = { ...tpl.content, sheets: freshSheets, activeSheet: tpl.content?.activeSheet ?? 0 }
      } else {
        tpl.content = {
          ...tpl.content,
          sheets: savedSheets.map((sheet, i) => {
            const fresh = freshSheets[i] || freshSheets[0]
            if (!fresh?.images?.length) return sheet
            if ((sheet.images || []).some(img => String(img.src || '').startsWith('data:image') && img.src.length >= 200)) return sheet
            return { ...sheet, images: fresh.images }
          })
        }
      }
      tpl.design = { ...(tpl.design || {}), ...(parsed.design || {}) }
    ...
    console.warn('[upload-templates] refresh from file failed', error?.message || error)
  }
  return tpl
}
...
export function registerUploadDocRoutes(app) {
  ...
  /** Save Word/Excel bytes only — open as-is in the document editor (no layout parse). */
  app.post('/api/open-editor', (req, res) => {
    upload.single('file')(req, res, async (err) => {
      ...
      try {
        ...
        if (!kind) {
          return res.status(400).json({ error: 'Only Word (.docx) and Excel (.xlsx / .xlsm) are supported.' })
        }
        const fileName = req.file.originalname || (kind === 'excel' ? 'workbook.xlsx' : 'document.docx')
        ...
        console.info('[open-editor] file stored', { fileId, kind, bytes: req.file.buffer.length, userId: req.userId })
        res.json({
          fileId,
          fileName,
          kind,
          mimeType: req.file.mimetype || 'application/octet-stream',
          officeEnabled: onlyOfficeEnabled()
        })
      } catch (error) {
        console.error('[open-editor] upload failed', error)
        const status = error?.status || 500
        res.status(status).json({
          error: error?.message || 'Could not open this file.',
          code: error?.code
        })
      }
    })
    ...
    upload.single('file')(req, res, async (err) => {
      ...
      try {
        ...
        try {
          ...
          if (kind === 'excel' && onlyOfficeEnabled()) {
            doc = {
              type: 'excel',
              fileName,
              sheets: [],
              design: { pageWidthPx: inferTemplatePageWidth('excel', [], {}) },
              warnings: [
                `Preview grid unavailable (${parseError?.message || 'parse failed'}). The native Excel editor will still open your file.`
              ]
            }
          } else if (kind === 'excel') {
            throw new Error(
              'Could not read this Excel file. Open it in Excel and use Save As → .xlsx, or start OnlyOffice (see docker-compose.onlyoffice.yml) for full layout support.'
            )
          } else {
            throw parseError
          }
        ...
        doc.fileId = fileId
        doc.renderer = onlyOfficeEnabled()
          ? 'onlyoffice'
          : (kind === 'word' ? 'docx-preview' : 'excel-grid')

        if (kind === 'word') {
          doc.mapping = scrubWordHtml(doc.html || '').mapping
        } else {
          const mapped = scrubExcelSheets(doc.sheets || [])
          doc.sheets = mapped.sheets
          doc.mapping = mapped.mapping
        }
        if (doc.mapping) {
          doc.mapping.summary = summarizeTemplateMapping(
            doc.mapping,
            kind === 'excel' ? doc.sheets : null
          )
        }
